...
class PreRank:
    def __init__(self, cfg) -> None:
        ...
        if self.method != "random":
            self.index = self.create_index(cfg) # 创建索引，包含训练集的id和embeddings
    ...
    def find(self):
        res_list = self.forward(self.dataloader)  # 验证集
        data_list = []
        starttime = datetime.datetime.now()
        for entry in res_list:
            data = self.dataset_reader.dataset_wrapper[entry['metadata']['id']]
            ctxs, ctxs_candidates = self.search(entry)  # 对于验证集中的每个样例，去训练集中检索最相似的k个样例
            data['ctxs'] = ctxs
