...
if __name__ == '__main__':
    input_path = 'output/scierc/mixtral_instruct_mixtral.json'
    score = evaluate_ner_list(input_path)
    with open(input_path.replace('.json', '.csv'), 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        # 写入表头
        writer.writerow(['Category', 'Precision', 'Recall', 'F1 Score'])
        # 写入数据行
        for category, prf in score.items():
            precision = prf['Precision']
            recall = prf['Recall']
            f1_score = prf['F1 Score']
            writer.writerow([category, precision, recall, f1_score])