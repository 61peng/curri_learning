...
def evaluate(self, input_texts: List[str], span=False) -> Tuple:
    ...
    #     invalid_indices = input_ids[(input_ids < 0) | (input_ids >= vocab_size)]
    #     print(f"Invalid indices found: {invalid_indices}")
    # print(f"input_ids: {input_ids[:10]}")  # 打印前10个输入
    # print(f"input_ids shape: {input_ids.shape}, dtype: {input_ids.dtype}, device: {input_ids.device}")

    # for key, value in inputs.items():
    #     print(f"{key} shape: {value.shape}, dtype: {value.dtype}, device: {value.device}")
    # # 添加断言检查
    # assert inputs['input_ids'] is not None, "input_ids should not be None"
    # assert inputs['input_ids'].dtype == torch.long, f"Expected input_ids of type long, but got {inputs['input_ids'].dtype}"
    # assert inputs['input_ids'].device == self.embed_tokens.weight.device, "input_ids and embedding weights must be on the same device"
...
def get_score(self, batch_labels, method='mdl', span=False, prior_loss_list=None):
    ...
    for i, batch in enumerate(batch_labels):
        with torch.no_grad():
            ce_loss, lens = evaluate(self, batch, span=span)  # 一个batch表示6中Demonstrations的组合方式
