...
label2text = get_mapping_token("scinli")
label_list = ['reasoning', 'entailment', 'contrasting', 'neutral']
...
def get_ALL(entry):
    return f"sentence1: {entry['sentence1']}\tSentence2: {entry['sentence2']}" if 'ALL' not in entry.keys() else entry['ALL']
...
class SciciteDatasetWrapper(DatasetWrapper):
    ...
    def __init__(self, dataset_path=None, dataset_split=None, ds_size=None):
        ...
        super().__init__()
        self.task_name = "scinli"
        self.field_getter = field_getter
        self.postfix = "It is"  # for inference
        if dataset_path is None:
            self.dataset = load_dataset("data/scinli", split=dataset_split)
